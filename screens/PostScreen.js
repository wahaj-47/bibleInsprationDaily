...
export default function PostScreen(props) {
	...
	function getComments() {
		axios
			.get(
				`${Wordpress.url}/wp-json/wp/v2/comments?post=${post.id}&per_page=100`
			)
			.then((response) => {
	...
	useEffect(() => {
		...
		getLikedPosts();
		axios
			.get(`${Wordpress.url}/wp-json/wp/v2/posts/${post.id}`)
			.then((response) => {
					...
					<TouchableOpacity
						...
						onPress={async () => {
							const cookie = await axios.post(
								Wordpress.url +
									"/api/user/generate_auth_cookie?email=appUser@email.com&password=YT@$sqZX29xT58H#yy#qKJuh"
							);
							...
							if (likedPosts.includes(post.id) || isLiked) {
								...
								axios
									.post(
										`${Wordpress.url}/api/user/like?cookie=${
											cookie.data.cookie
					...
					<View>
						<TouchableOpacity
							...
							onPress={async () => {
								...
								axios
									.post(
										`${Wordpress.url}/api/user/post_comment?cookie=${cookie.data.cookie}&post_id=${post.id}&content=${comment}&comment_status=hold`
									)
									.then(() => {
										getComments();
										setComment("");
